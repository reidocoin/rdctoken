...
/** Fees smaller than this (in urdct) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight > 0 && nHeight <= Params().LAST_POW_BLOCK())
        nSubsidy = 18750 * COIN;
    else if (GetTime() > Params().StartMasternodePayments()) // passou 20/09 15:00
        nSubsidy = 1.5 * COIN;
    else
        nSubsidy = 0;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (blockValue == 0 || GetTime() < Params().StartMasternodePayments())
        return 0;

    // Check if we reached coin supply
    ret = blockValue; // 100% of block reward
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // BIP16 didn't become active until RDCT 1 2012
